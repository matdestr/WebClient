...
public class SessionGameServiceImpl implements SessionGameService {
    ...
    private AsynchronousSessionSchedulerService asynchronousSessionSchedulerService;
    ...
    @Autowired
    public void setAsynchronousSessionSchedulerService(AsynchronousSessionSchedulerService asynchronousSessionSchedulerService) {
        this.asynchronousSessionSchedulerService = asynchronousSessionSchedulerService;
    }
    ...
    @Override
    public void inviteNonExisitingUserForSession(Session session, String email) {
        if (session.getSessionStatus() != SessionStatus.CREATED) {
            throw new SessionGameServiceException("Cannot invite user when the session has already started");
        }

        if (session.getParticipantInfo().stream().anyMatch(p -> p.getParticipant().getEmail().equals(email))) {
            throw new SessionGameServiceException("User has already been invited to join the session");
        }

        ParticipantInfo participantInfo = new ParticipantInfo();

        //todo invitation maken

        session.getParticipantInfo().add(participantInfo);
        session = sessionService.updateSession(session);
        emailService.sendSessionInvitationToUnexistingUser(session, session.getOrganizer(), email);
    }
    ...
    public ParticipantInfo getNextParticipant(Session session) {
        return getNextParticipant(session, session.getCurrentParticipantPlaying());
    }

    @Override
    public ParticipantInfo getNextParticipant(Session session, ParticipantInfo participantInfo) {
        ParticipantInfo currentParticipant = participantInfo;
        if (currentParticipant != null) {
            ...
            if (currentParticipant.getJoinNumber() == max) {
                ...
                for (int i = 0; i < participantSequence.size(); i++) {
                    if (participantSequence.get(i).getJoinNumber() == currentParticipant.getJoinNumber())
                        currentIndex = i;
                }
    ...
    public CardPosition increaseCardPriority(Session session, User user, CardDetails cardDetails) {
        ...
        if (session.getCurrentParticipantPlaying().getParticipant().getUserId() != user.getUserId()) {
            throw new SessionGameServiceException("It's isn't your turn");
        }

        if (!session.getCardPositions().stream().anyMatch(c -> c.getCardDetails().getCardDetailsId() == cardDetails.getCardDetailsId())) {
            throw new SessionGameServiceException("Card is not in the game");
        }
        ...
        if (cardPositionOptional.isPresent()) {
            ...
            ParticipantInfo currentParticipant = session.getCurrentParticipantPlaying();
            ParticipantInfo nextParticipant = getNextParticipant(session);
            ...
            if (session instanceof AsynchronousSession){
                ...
                asynchronousSessionSchedulerService.cancelParticipantNotification(asynchronousSession, currentParticipant.getParticipant());
                asynchronousSessionSchedulerService.cancelNextParticipantAssignment(asynchronousSession);
                asynchronousSessionSchedulerService.scheduleNextParticipantAssignment(asynchronousSession);
                asynchronousSessionSchedulerService.scheduleParticipantNotificationTurnAboutToExpire(asynchronousSession, getNextParticipant(session).getParticipant());
            }
    ...
    public void endGame(Session session) {
        if (session.getSessionStatus() == SessionStatus.IN_PROGRESS) {
            if (session instanceof AsynchronousSession){
                AsynchronousSession asynchronousSession = (AsynchronousSession) session;
                asynchronousSessionSchedulerService.cancelParticipantNotification(asynchronousSession, asynchronousSession.getCurrentParticipantPlaying().getParticipant());
                asynchronousSessionSchedulerService.cancelNextParticipantAssignment(asynchronousSession);
            }
